...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        # data split loading
        assert self.split in ['train', 'val', 'trainval', 'test', 'check', 'check2']
        ...
        self.sam_region_dir = os.path.join(self.data_dir, 'label_sam', 'region')
        self.sam_depth_dir = os.path.join(self.data_dir, 'label_sam', 'depth')
    ...
    def __getitem__(self, item):
        ...
        if self.data_augmentation:
            ...
            if np.random.random() < self.random_flip:
                ...
                sam_region = sam_region.transpose(Image.FLIP_LEFT_RIGHT)
                sam_depth = sam_depth.transpose(Image.FLIP_LEFT_RIGHT)
                sam_region = np.array(sam_region)
                sam_depth = np.array(sam_depth)
        ...
        if random_mix_flag == True:
            ...
            while count_num < 50:
                ...
                sam_region2 = np.load(os.path.join(self.sam_region_dir, '%06d.npy' % random_index))
                sam_depth2 = np.load(os.path.join(self.sam_depth_dir, '%06d.npy' % random_index))
                    ...
                    if dst_W_temp == dst_W and dst_H_temp == dst_H:
                        ...
                        if len(objects_1) + len(objects_2) < self.max_objs:
                            ...
                            if random_flip_flag == True:
                                ...
                                sam_region2 = Image.fromarray(sam_region2)
                                sam_depth2 = Image.fromarray(sam_depth2)
                                sam_region2 = sam_region2.transpose(Image.FLIP_LEFT_RIGHT)
                                sam_depth2 = sam_depth2.transpose(Image.FLIP_LEFT_RIGHT)
                                sam_region2 = np.array(sam_region2)
                                sam_depth2 = np.array(sam_depth2)

                            sam_region = sam_region + sam_region2
                            sam_region = np.where(sam_region >= 1., 1., 0.)
                            # 두 개 중 min 값을 선택 (겹치는 곳만)
        ...
        sam_region = Image.fromarray(sam_region)
        sam_depth = Image.fromarray(sam_depth)
        sam_region = sam_region.transform(tuple(self.resolution.tolist()),
                                          method=Image.AFFINE,
                                          data=tuple(trans_inv.reshape(-1).tolist()),
                                          resample=Image.NEAREST)
        
        sam_region = np.array(sam_region).astype(np.float32)
        sam_depth = sam_depth.transform(tuple(self.resolution.tolist()),
                                        method=Image.AFFINE,
                                        data=tuple(trans_inv.reshape(-1).tolist()),
                                        resample=Image.NEAREST)  # Nearest임
        sam_depth = np.array(sam_depth).astype(np.float32)
        if self.aug_crop:
            sam_depth = sam_depth * crop_scale
        ...
        for i in range(object_num):
            ...
            boxes_2d[i] = xmin, ymin, xmax, ymax
        ...
        if random_mix_flag == True:
            ...
            for i in range(object_num_temp):
                ...
                boxes_2d[i + object_num] = xmin, ymin, xmax, ymax
        ...
        targets = {
            ...
            'mask_2d': mask_2d,
            'obj_region': obj_region,  # sam_region
            'depth_map': sam_depth,
            'boxes_2d': boxes_2d,
