...
class MonoCLUE(nn.Module):
    ...
    def forward(self, images, calibs, img_sizes, dn_args=None):
        ...
        intermediate_output = self.det2d_transformer(srcs, masks, pos, seg_embed, query_embeds)
...
class SetCriterion(nn.Module):
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes):
        ...
        gt_sam_depth = torch.cat([t['depth_map'].unsqueeze(0) for t in targets], dim=0)
        gt_sam_depth = F.interpolate(gt_sam_depth.unsqueeze(1).float(), size=depth_map_logits.shape[2:], mode='nearest')
        gt_sam_depth = gt_sam_depth.squeeze(1)

        losses = dict()

        losses["loss_depth_map"] = self.ddn_loss(
            depth_map_logits, gt_boxes2d, num_gt_per_img, gt_center_depth, gt_sam_depth)
        return losses
    ...
    def loss_region(self, outputs, targets, indices, num_boxes):
        ...
        gt_region = torch.cat([t['sam_region'].unsqueeze(0) for t in targets], dim=0)

        loss = 0
        losses = dict()
        for region_prob in region_probs:
            gt_region_resized = F.interpolate(gt_region.unsqueeze(1).float(), size=region_prob.shape[2:],
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    if loss == 'depth_map' or loss == 'region' or loss == 'codebook':
                        continue
                    kwargs = {}
                    if loss == 'labels':
