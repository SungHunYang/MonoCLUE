...
class MSDeformAttn_Relocalization(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, conditional=False):
        """
        Multi-Scale Deformable Attention Module
        :param d_model      hidden dimension
        ...
        self.sampling_offsets = nn.Linear(d_model + 1, n_heads * n_levels * n_points * 2)
        self.attention_weights = nn.Linear(d_model + 1, n_heads * n_levels * n_points)
        self.conditional = conditional
        if conditional:
            self.value_proj = nn.Linear(d_model//2, d_model//2)
            self.output_proj = nn.Linear(d_model//2, d_model//2)
